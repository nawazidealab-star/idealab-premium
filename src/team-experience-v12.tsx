...
const ROLE_HINTS:Record<Exclude<Role,'super_admin'>,string>={
  admin:'Operations lead — broad access to clients, projects, tasks, content, invoices and reports.',
  sales:'Sales — leads and client records, with no finance or delivery modules by default.',
  project_manager:'Project manager — clients, projects, tasks and content delivery.',
  finance:'Finance — clients, invoices and financial reports.',
  content:'Content — clients and content planning.'
};
...
function AccessManager({user,onClose,onSaved,toast}:{user:AccessUser;onClose:()=>void;onSaved:()=>void;toast:(t:string,e?:boolean)=>void}){
  const[access,setAccess]=useState({...user.access});const[selected,setSelected]=useState<number[]>(user.client_ids||[]);const[clients,setClients]=useState<ClientRow[]>([]);const[query,setQuery]=useState('');const[saving,setSaving]=useState(false);const[error,setError]=useState('');
  useEffect(()=>{void adminApi<{results:ClientRow[]}>('/api/clients').then(r=>setClients(r.results||[])).catch(e=>setError(e instanceof Error?e.message:'Could not load clients.'))},[]);
  const filtered=useMemo(()=>clients.filter(c=>`${c.name} ${c.company||''}`.toLowerCase().includes(query.toLowerCase())),[clients,query]);
  const save=async()=>{setSaving(true);setError('');try{await adminApi(`/api/access/users/${user.id}`,{method:'PATCH',body:JSON.stringify({access,all_clients:access.all_clients,client_ids:access.all_clients?[]:selected})});onSaved()}catch(e){const m=e instanceof Error?e.message:'Could not save access.';setError(m);toast(m,true)}finally{setSaving(false)}};
  const toggle=(id:number,checked:boolean)=>setSelected(v=>checked?Array.from(new Set([...v,id])):v.filter(x=>x!==id));
  return <Modal onClose={onClose} access><ModalHead title={`Access · ${user.name}`} onClose={onClose}/><div className="il-team-access-guide"><div><strong>Give this person only what they need for their job.</strong><small>{user.email} · {user.role.replaceAll('_',' ')}. Their role sets the maximum permissions; you can narrow them here.</small></div><div className="il-team-flow"><span>1 Account</span><i>→</i><b>2 Access</b><i>→</i><span>3 Ready</span></div></div><div className="il-v4-access-editor"><section><h4>Workspace modules</h4>{MODULES.map(m=><label key={m}><input type="checkbox" checked={!!access[m]} onChange={e=>setAccess({...access,[m]:e.target.checked})}/><span>{MODULE_LABELS[m]}</span></label>)}</section><section><h4>Client visibility</h4><label><input type="checkbox" checked={access.all_clients} onChange={e=>setAccess({...access,all_clients:e.target.checked})}/><span>All clients</span></label>{!access.all_clients&&<><div className="il-access-client-tools"><input className="il-access-client-search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search clients…"/><button type="button" onClick={()=>setSelected(clients.map(c=>c.id))}>Select all</button><button type="button" onClick={()=>setSelected([])}>Clear</button></div><div className="il-v4-client-checks">{filtered.map(c=><label key={c.id}><input type="checkbox" checked={selected.includes(c.id)} onChange={e=>toggle(c.id,e.target.checked)}/><span>{c.name}{c.company?` · ${c.company}`:''}</span></label>)}{!filtered.length&&<small>No clients match your search.</small>}</div></>}</section></div>{error&&<div className="il-team-api-error" style={{margin:'8px 26px 0'}}>{error}</div>}<div className="il-v4-modal-actions"><button type="button" className="il-admin-button" onClick={onClose}>Finish later</button><button type="button" className="il-admin-button il-admin-primary" disabled={saving||(!access.all_clients&&selected.length===0)} onClick={()=>void save()}>{saving?'Saving…':<><Check size={15}/> Save access</>}</button></div></Modal>
}
